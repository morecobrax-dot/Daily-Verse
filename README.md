...
that cache: `data/curation.json` names references and themes, and every
character of text comes from the corpus. A build that cannot derive every
passage writes nothing at all.
...
data/curation.json       which passages are readings, and their theme tags
